...
Download the `.qmd` to work through in VS Code:
...
You learned the four-step wrangling pipeline in the [Data Wrangling chapter](../../workbook/04-data-wrangling.qmd) using the Twitch dataset. That chapter taught you the functions. This practice tests whether you can *apply* them: to data you haven't seen, with column names you don't recognize, and problems you have to find yourself.

There is a second difference, and it is a useful one. The Twitch data arrived through `twitch_chat()` and `twitch_streams()`, functions that reach into the `v2v` package and hand you the tables with no file path to get wrong. Almost no data in the world arrives that conveniently. This one comes from a URL, which is much closer to the ordinary case.
...
`read_csv()` works with URLs, not just local files. R downloads the data directly into memory. This is how many open datasets are shared, with no file download needed.
:::
...
18 primary types. Water is the most common. This is a clean categorical variable, with no inconsistent labels and no typos. Not every dataset will be this cooperative.
...
Generations 1 and 5 have the most Pokemon. This column is numeric, but it represents a category, and a good candidate for `factor()`.
...
| No derived variables | (none yet) | Create `power_tier` and `size_class` with `case_when()` |
...
In the Twitch dataset, a missing `game` meant the study genuinely could not say what a channel streamed, so those 501 messages kept an honest `NA` and sat out the comparison. Here, a missing `type_2` means something different: the Pokemon only has one type, and the absence is itself the information. The decision to fill it with `"none"` rather than leave it as `NA` depends on your analysis plan. We fill it here so `count()` and `factor()` work cleanly.

The lesson is that `NA` is not one thing. Before you decide how to handle a missing value, work out what its absence means.
:::
...
The Data Wrangling assignment uses the Twitch data from the `v2v` package and follows this exact pipeline. If you completed this practice, you've already done the hard part: same functions, same order, different column names.
:::
