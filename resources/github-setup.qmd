...
- R
- Git & GitHub
- VS Code (editor and course workspace)
...
2. Run the installer and accept the default options. It asks a lot of questions and **Next**
   is the right answer to all of them.
3. **Quit VS Code completely and reopen it.** Not just the terminal, and not Reload
   Window: VS Code reads your PATH once when it starts, so anything installed while it was
   running stays invisible to it until the program itself restarts.
4. Open a terminal in VS Code (**Terminal, New Terminal**) and run `git --version` to confirm.

**Mac:** there is nothing to download.

1. Open **Terminal** and run `git --version`.
2. If a box appears offering to install the command line developer tools, click **Install**
   and wait for it to finish. That box is Git arriving.
3. Run `git --version` again. You should see a version number.
4. **Quit VS Code completely with `Cmd+Q` and reopen it.** Closing the window is not enough
   on a Mac, and VS Code will not see Git until the program itself restarts.

You do not need Homebrew for this. If you have seen `brew install git` suggested elsewhere,
ignore it: Homebrew is a separate thing you would have to install first, and macOS gives you
Git without it.
...
### 4.2 Tell Git Who You Are

Git stamps every commit with a name and an email address, and it refuses to
commit until it knows them. Run these two commands once, in the same terminal:
...
Use an address that is attached to your GitHub account, either the one you
signed up with or one you added under **Settings, Emails**. A commit made with
any other address still saves, but GitHub cannot match it to your profile, so
your work shows up under a stranger's name. That matters in this course,
because your repository is how your work is read and graded.

`--global` means once per computer, not once per repository. To see what Git
already has, run `git config --global --list`.

**If you skip this step**, your first commit stops with `Author identity
unknown` and `Please tell me who you are`. That message is not about your
GitHub password or your login. It means these two commands have not been run
yet. Run them, then commit again.
